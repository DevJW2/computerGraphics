...
def add_sphere( edges, cx, cy, cz, r, step ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            indexB = index + 1
            begin =  (lat + 1) * step + longt
            end = (lat - 1) * step + longt + 1

            if lat == lat_start: 
                if (longt + 1 == longt_stop):
	                end = lat_start
                else:
                    end = (lat_stop - 1) * step + longt + 1
            if lat + 1 == lat_stop: 
                begin = longt 
            if indexB == lat_stop * step: 
                indexB = 0

            add_polygon(edges, 
                        points[index][0], points[index][1], points[index][2],
                        points[indexB][0], points[indexB][1], points[indexB][2],
                        points[begin][0], points[begin][1], points[begin][2])
            add_polygon(edges,
                        points[indexB][0], points[indexB][1], points[indexB][2],
                        points[index][0], points[index][1], points[index][2],
